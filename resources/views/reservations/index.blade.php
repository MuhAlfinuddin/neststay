    ...
    <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
        <form action="{{ route('reservations.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-grow">
                ...
                <input id="search" name="search" type="text" placeholder="Cari nama tamu..." value="{{ request('search') }}" class="appearance-none block w-full px-3 py-2 border border-slate-200 rounded-xl placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-xs">
            </div>
                                ...
                                <td class="px-6 py-4 text-right space-x-1">
                                    @if ($res->status === 'pending' || $res->status === 'confirmed')
                                        <form action="{{ route('reservations.check-in', $res->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="px-2.5 py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition">
                                                Check In
                                            </button>
                                        </form>
